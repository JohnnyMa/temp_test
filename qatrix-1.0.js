...
(function(window, document, undefined) {
  ...
  var version = '1.0', rbline = /(^\n+)|(\n+$)/g, rbrace = /^(?:\{.*\}|\[.*\])$/, rcamelCase = /-([a-z])/ig, rdigit = /\d/, rline = /\r\n/g, rnum = /[\-\+0-9\.]/ig, rspace = /\s+/, rtrim = /(^\s*)|(\s*$)/g, ropacity = /opacity=([^)]*)/, rvalidchars = /^[\],:{}\s]*$/, rvalidescape = /\\(?:["\\\/bfnrt]|u[\da-fA-F]{4})/g, rvalidtokens = /"[^"\\\r\n]*"|true|false|null|-?(?:\d\d*\.|)\d+(?:[eE][\-+]?\d+|)/g, rvalidbraces = /(?:^|:|,)(?:\s*\[)+/g, animDisplay = ['height', 'margin-top', 'margin-bottom', 'padding-top', 'padding-bottom'],
  ...
  nodeManip = function(elem, node) {
    ...
    if (type === 'string') {
      var doc = elem && elem.ownerDocument || document, fragment = doc.createDocumentFragment(), div = $new('div'), ret = [];
  ...
  displayElem = function(elem, type, duration, callback) {
    return mapcall(elem, function(elem) {
      var prop = {}, show = type === 'show', style = elem.style, display, temp, overflow;
  ...
  //  style
  //  Qatrix = {key1: value1, key2: value2, ...};
  //
  ...
  Qatrix = {
    ...
    $class : document.getElementsByClassName ? function(elem, className, callback) {
      return mapcall(elem.getElementsByClassName(className), callback);
    } : function(elem, className, callback) {
      var match = [], rclass = new RegExp('\\b' + className + '\\b');
    ...
    $select : document.querySelectorAll ? function(selector, callback) {
    ...
    function(selector, callback) {
      var style = Qatrix.Qselector.styleSheet, match = [];
    ...
    $data : {
      get : function(elem, name) {
        ...
        return value === "true" ? true : value === "false" ? false : value === "null" ? '' : value === null ? '' : value === '' ? '' : !isNaN(parseFloat(value)) && isFinite(value) ? +value : rbrace.test(value) ? $json.decode(value) : value;
      },
      set : function(elem, name, value) {
        return mapcall(elem, function(elem) {
    ...
    $event : {
      ...
      remove : document.removeEventListener ? function(elem, type, handler) {
        return mapcall(elem, function(elem) {
          if ( typeof type === 'object') {
            $each(type, function(type, handler) {
    ...
    $style : {
      get : window.getComputedStyle ? function(elem, property) {
        if (elem !== null) {
        ...
        if (elem !== null) {
          var value = property === 'opacity' ? ropacity.test(elem.currentStyle['filter']) ? (0.01 * parseFloat(RegExp.$1)) + '' : 1 : elem.currentStyle[$string.camelCase(property)];
          return value === 'auto' ? 0 : value;
        }
        return false;
      },
      set : document.documentElement.style.opacity !== undefined ? function(elem, name, value) {
        return mapcall(elem, function(elem) {
    ...
    $offset : function(elem) {
      var body = document.body, doc_elem = document.documentElement, box = elem.getBoundingClientRect();
      return {
    ...
    $text : function(elem, text) {
      ...
      return mapcall(elem, function(elem) {
        if (text) {
          ...
          var rtext = '', textContent = elem.textContent, nodeType;
    ...
    $animate : ( function() {
      ...
      var style = document.documentElement.style, prefix_name = style.webkitTransition !== undefined ? 'Webkit' : style.MozTransition !== undefined ? 'Moz' : style.OTransition !== undefined ? 'O' : style.MsTransition !== undefined ? 'ms' : '', transition_name = prefix_name + 'Transition', transform_name = prefix_name + 'Transform';
      return function(elem, properties, duration, callback) {
        return mapcall(elem, function(elem) {
          var css_value = [], css_name = [], unit = [], css_style = [], style = elem.style, css, offset;
      ...
      return mapcall(elem, function(elem) {
        var step = 0, i = 0, j = 0, length = 0, p = 30, css_to_value = [], css_from_value = [], css_name = [], css_unit = [], css_style = [], property_value, css, offset, timer;
    ...
    $cookie : {
      get : function(key) {
        var cookies = document.cookie.split('; '), i = 0, l = cookies.length, temp, value;
        for (; i < l; i++) {
    ...
    $json : {
      //优先使用浏览器提供的接口
      decode : window.JSON ? function(data) {
      ...
      encode : window.JSON ? function(data) {
    ...
    $ajax : function(url, options) {
      //when only one param given, then the param should be options
      if ( typeof url === 'object') {
        options = url;
        url = undefined;
      }
      options = options || {};
      var request = XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP'), param = [], response;
